...
import event_lang.dsl.{AbstractChannelImp, AbstractChannelType}
import event_lang.network.LHChannel.LHostSessionChannelEPs
import event_lang.types.{Role, RoleSet}
import example.sparkCluster.SparkCM.M.GStartEx_m_M.SndExRunning
...
object ClusterRunner {
  def main(args: Array[String]): Unit = {
    ...
    val block: (__EPType_W, StateW, AbstractChannelImp) => Boolean = {
      case (SndExDone, s, c) =>
        s.sIdToAppId(c.session.sesId) == 1 && numKills < numToKillW
      case _ => false
    }
    ...
    val cusCodeM: (__EPType_M, StateMastr, AbstractChannelImp) => Unit = {
      case (GStartEx_m_M.RcvExStarted, s, c) if s.sesIdToApp(c.session.sesId) == 1 && numKills < numToKillW =>
        numKills += 1
    ...
    val res = EPRunner.bootstrap(mainId, pickId, 0,0,
      (() => new EP_ZK(), List(0)), (() => new EP_Master(sState = new StateMastr(apps.toList), customCode = cusCodeM), List(1, 2)),
      (() => new EP_Worker(customCode = cusCodeW, block = block), List(3, 4, 5, 6, 7, 8)))
    network = res._1
