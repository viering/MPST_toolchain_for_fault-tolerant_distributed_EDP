...
protected case class __HdlImp(private val c : AbstractChannel, session : Session) extends AbstractChannelImp {
  ...
  override protected def __children: List[EPPingPong_bb_B] = List(End_bb_B_PingPong)
  override type implT = __SndStartImp
  override type implNextT = __End_bb_B_PingPongImp
...
def sndToa_A(m : MESSAGES.PingPong.Start) : __End_bb_B_PingPongImp = {
assert(notUsed, s"The channel send musted be used linear")
 notUsed=false
c.snd(Role("a",RoleSet("A")),m)
__End_bb_B_PingPongImp(c,session)}
def !(m : MESSAGES.PingPong.Start) : __End_bb_B_PingPongImp = {
assert(notUsed, s"The channel send musted be used linear")
 notUsed=false
c.snd(Role("a",RoleSet("A")),m)
__End_bb_B_PingPongImp(c,session)}
def snd(m : MESSAGES.PingPong.Start) : __End_bb_B_PingPongImp = {
assert(notUsed, s"The channel send musted be used linear")
 notUsed=false
c.snd(Role("a",RoleSet("A")),m)
__End_bb_B_PingPongImp(c,session)}
...
protected  trait End_bb_B_PingPong extends EPPingPong_bb_B with event_lang.dsl.ChannelTypeEnd
protected  object End_bb_B_PingPong extends End_bb_B_PingPong {
  override protected def __children: List[EPPingPong_bb_B] = List()
  override type implT = __End_bb_B_PingPongImp
  override type implNextT = Nothing
override def toString() : String = {"EPPingPong_bb_B.End_bb_B_PingPong"}
  
  override protected def __create(c : AbstractChannel, session : Session) : implT = __End_bb_B_PingPongImp(c,session)}

protected case class __End_bb_B_PingPongImp(private val c : AbstractChannel, session : Session) extends AbstractChannelImp {
  override def from : AbstractChannelType = {
    End_bb_B_PingPong
}
...
protected  object RecT extends RecT {
  override protected def __children: List[EPPingPong_b_B] = List(RcvPing)
...
protected  object RecT extends RecT {
  override protected def __children: List[EPPingPong_a_A] = List(SndPing)
  override type implT = __RecTImp
...
protected case class __SndPingImp(private val c : AbstractChannel, session : Session) extends AbstractChannelImp {
  ...
  private var notUsed = true
def sndTob_B(m : MESSAGES.PingPong.Ping) : __RcvPongImp = {
assert(notUsed, s"The channel send musted be used linear")
 notUsed=false
c.snd(Role("b",RoleSet("B")),m)
__RcvPongImp(c,session)}
def !(m : MESSAGES.PingPong.Ping) : __RcvPongImp = {
assert(notUsed, s"The channel send musted be used linear")
 notUsed=false
c.snd(Role("b",RoleSet("B")),m)
__RcvPongImp(c,session)}
def snd(m : MESSAGES.PingPong.Ping) : __RcvPongImp = {
assert(notUsed, s"The channel send musted be used linear")
 notUsed=false
c.snd(Role("b",RoleSet("B")),m)
__RcvPongImp(c,session)}
