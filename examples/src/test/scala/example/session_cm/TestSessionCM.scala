...
import example.sparkCluster.{EP_Master, EP_Worker, EP_ZK, SimpleSumDriver, StateMastr, StateW}
...
class TestSessionCM extends AnyFunSuite with TimeLimitedTests with TimeLimits{
  ...
  test("Normal Execution; One Application to run") {
    Console.withOut(new VoidOutStream) {
      Console.withErr(new VoidOutStream) {
        ...
        val sState: StateMastr = new StateMastr(List(new SimpleSumDriver(2, x => {
        ...
        val cusCodeM: (__EPType_M, StateMastr, AbstractChannelImp) => Unit = {
          case (GStartEx_m_M.RcvExStarted, s, c) =>
          case _ => {}
        }
        val blockM: (__EPType_M, StateMastr, AbstractChannelImp) => Boolean = {
          case (_, _, _) => false
          case _ => false
        }
  ...
  test("Normal Execution; 2 Application to run") {
    Console.withOut(new VoidOutStream) {
      Console.withErr(new VoidOutStream) {
        val mainId = 0
        val pickId = 1
        ...
        val sState: StateMastr = new StateMastr(List(
  ...
  test("Normal Execution; N (random) Application to run") {
    Console.withOut(new VoidOutStream) {
      Console.withErr(new VoidOutStream) {
        val mainId = 0
        val pickId = 1
        ...
        val sState: StateMastr = new StateMastr(driver.toList)
  ...
  test("Normal Execution; N (rnd) Application and M (rnd) worker") {
    Console.withOut(new VoidOutStream) {
      Console.withErr(new VoidOutStream) {
        val mainId = 0
        val pickId = 1
...
//
//  test("Kill one rnd worker - one application") {
//    for (i <- 0 to 12) {
...
//
//
//  test("Kill 2 rnd worker - 2 application") {
